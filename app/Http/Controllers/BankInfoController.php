...
class BankInfoController extends Controller
{
    ...
    private function validation(array $data)
    {
        return Validator::make($data, [
            'name' => 'required|unique:bank_info',
        ]);
    }
    ...
    private function new_rec_create(array $recs){
        ...
        foreach($recs as $rec){
            ...
            if(BankInfo::where('name', $rec['name'])->where('ifsc', $rec['ifsc'])->count() == 0) {
                //Save data in DB
                $record = new BankInfo();
                $record->name = $rec['name'];
                $record->ifsc = $rec['ifsc'];
    ...
    private function rec_update(array $recs)
    {
        ...
        foreach ($recs as $rec) {
            //Check for duplicate entry
            //if(BankInfo::where('name', $rec['name'])->count() == 0) {
                //Update data in DB
                $record = BankInfo::find($rec['id']);
                $record->name = $rec['name'];
                $record->ifsc = $rec['ifsc'];
                $record->address = $rec['address'];
    ...
    public function index(){
        $bankInfoes = BankInfo::getByPaginate(8);
        //dd($bankInfoes);
        $pageSetting = ["page" => "1", "recs" => 8, "sort_type" => "asc", "sort_by" => "id", "query" => ""];
        return view('admin.bankInfo', compact('bankInfoes'))->with('pageSetting', $pageSetting);
    }
    ...
    public function fetch_data(Request $request)
    {
        ...
        $bankInfoes = BankInfo::getByConditionalPaginate($recs, $sort_by, $sort_type, $query);
        //dd($bankInfoes);
