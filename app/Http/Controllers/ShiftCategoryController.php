...
class ShiftCategoryController extends Controller
{
    ...
    private function validation(array $data)
    {
        return Validator::make($data, [
            'name' => 'required|unique:shift_types',
        ]);
    }
    ...
    private function new_rec_create(array $recs){
        ...
        foreach($recs as $rec){
            ...
            if(ShiftCategory::where('name', $rec['name'])->count() == 0) {
                //Save data in DB
                $record = new ShiftCategory();
    ...
    private function rec_update(array $recs)
    {
        ...
        foreach ($recs as $rec) {
            ...
            if(ShiftCategory::where('name', $rec['name'])->count() == 0) {
                //Update data in DB
                $record = ShiftCategory::find($rec['id']);
                $record->name = $rec['name'];
    ...
    public function index(){
        $shiftCategories = ShiftCategory::getByPaginate(8);
        //dd($shiftCategories);
        $pageSetting = ["page" => "1", "recs" => 8, "sort_type" => "asc", "sort_by" => "id", "query" => ""];
        return view('admin.shiftCategory', compact('shiftCategories'))->with('pageSetting', $pageSetting);
    }
    ...
    public function fetch_data(Request $request)
    {
        ...
        $shiftCategories = ShiftCategory::getByConditionalPaginate($recs, $sort_by, $sort_type, $query);
        //dd($shiftCategories);
