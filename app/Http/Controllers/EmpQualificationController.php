...
class EmpQualificationController extends Controller
{
    ...
    private function validation(array $data)
    {
        return Validator::make($data, [
            'name' => 'required|unique:emp_qualification',
        ]);
    }
    ...
    private function new_rec_create(array $recs){
        ...
        foreach($recs as $rec){
            ...
            if(EmpQualification::where('name', $rec['name'])->count() == 0) {
                //Save data in DB
                $record = new EmpQualification();
    ...
    private function rec_update(array $recs)
    {
        ...
        foreach ($recs as $rec) {
            ...
            //if(EmpQualification::where('name', $rec['name'])->count() == 0) {
                //Update data in DB
                $record = EmpQualification::find($rec['id']);
                $record->name = $rec['name'];
                $record->description = $rec['description'];
    ...
    public function index(){
        $empQualifications = EmpQualification::getByPaginate(8);
        //dd($empQualifications);
        $pageSetting = ["page" => "1", "recs" => 8, "sort_type" => "asc", "sort_by" => "id", "query" => ""];
        return view('admin.empQualification', compact('empQualifications'))->with('pageSetting', $pageSetting);
    }
    ...
    public function fetch_data(Request $request)
    {
        ...
        $empQualifications = EmpQualification::getByConditionalPaginate($recs, $sort_by, $sort_type, $query);
        //dd($empQualifications);
