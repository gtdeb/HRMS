...
@section('content')
        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <div class="table-responsive-sm" style="overflow-x:auto; overflow-y:auto;">
                    ...
                    <table cellspacing="0" rules="all" border="1" id="Table1" style="border-collapse:collapse;" class="table table-hover table-bordered" >
                        <thead>
                            ...
                            <tr align='center'>
                                ...
                                <th width="27%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="description" style="cursor: pointer"> Description <span id="description_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="30%" align='center'> In Use(count) </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr align='center'>
                            @if($empQualifications)
                                <?php $index = 2; ?>
                                @foreach ($empQualifications as $empQualification)
                                    <?php $index++; ?>
                                    <tr>
                                        @if($empQualification->in_use == 0)
                                        ...
                                        @endif
                                        <td>{{ $empQualification->id }}</td>
                                        <td><label class="lblname">{{$empQualification->name}}</label></td>
                                        <td><label class="lblname">{{$empQualification->description}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($empQualification->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $empQualification->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
                    ...
                    <div hidden>
                        <form id="searchfilter" method="POST" action="{{route('emp_qualification.fetch_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
                        ...
                        <form id="cud_action" method="POST" action="{{route('emp_qualification.update_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("emp_qualification.fetch_data") }}?page=' + page;
        ...
        function save(){
            ...
            for (var k = 0, n = 0, u = 0, d = 0, l = 0; k < actions.length; k++) {
                ...
                if (actions[k].innerHTML == 'L') {
                    ...
                    if(names[0].value) {
                        nArr[n++] = JSON.stringify({"name": names[0].value, "description": names[1].value});
                    }
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("emp_qualification.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
