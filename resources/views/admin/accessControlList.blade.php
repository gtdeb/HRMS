...
@section('content')
        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <div class="table-responsive-sm" style="overflow-x:auto; overflow-y:auto;">
                    ...
                    <table cellspacing="0" rules="all" border="1" id="Table1" style="border-collapse:collapse;" class="table table-hover table-bordered" >
                        <thead>
                            ...
                            <tr align='center'>
                                ...
                                <th width="27%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="access_level" style="cursor: pointer"> Name <span id="access_level_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="27%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="status" style="cursor: pointer"> Mode <span id="status_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                            ...
                            @if($accessControlList)
                                ...
                                @foreach ($accessControlList as $accessControl)
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <td>{{ $accessControl->id }}</td>
                                        <td><label class="lblname">{{$accessControl->access_level}}</label></td>
                                        <td><label class="lblname">{{$accessControl->status}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($accessControl->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $accessControl->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
                    ...
                    <div hidden>
                        <form id="searchfilter" method="POST" action="{{route('access_control.fetch_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
                        ...
                        <form id="cud_action" method="POST" action="{{route('access_control.update_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("access_control.fetch_data") }}?page=' + page;
        ...
        function add(){
            ...
            grid.insertCell(2).innerHTML = "<select id='access_level' class='name' style='text-align:center;" +
                "height:20px;'><option value='HR-Admin'>HR-Admin</option><option value='Employee'>Employee</option>" +
                "<option value='System'>System</option></select>";
            grid.insertCell(3).innerHTML = "<select id='status' class='name' style='text-align:center;" +
                "height:20px;'><option value='Active'>Active</option><option value='In-Active'>In-Active</option>" +
                "</select>";
        ...
        function edit(element, index){
            ...
            if(element.innerHTML.search("pencil") != -1){
                ...
                cells[2].innerHTML = "<label class='lblname' hidden>" + lblName + "</label><select id='access_level' " +
                    "class='name' style='text-align:center;height:20px;'><option value='HR-Admin'>HR-Admin</option>" +
                    "<option value='Employee'>Employee</option><option value='System'>System</option></select>";
                cells[2].querySelector('option[value=' + lblName + ']').selected = true;

                lblelement = cells[3].getElementsByClassName("lblname");
                lblName = lblelement[0].innerText;
                cells[3].innerHTML = "<label class='lblname' hidden>" + lblName + "</label><select id='status' " +
                ...
                cells[3].querySelector('option[value=' + lblName + ']').selected = true;
            }
            else{
                if(chkbx[0]) {
        ...
        function save(){
            ...
            for (var k = 0, n = 0, u = 0, d = 0, l = 0; k < actions.length; k++) {
                ...
                if (actions[k].innerHTML == 'L') {
                    ...
                    if(names[0].value) {
                        nArr[n++] = JSON.stringify({"access_level": names[0].value, "status": names[1].value});
                    }
                } else if (actions[k].innerHTML == 'LU' || actions[k].innerHTML == 'DU') {
                    //Existing record update request
                    var names = row.getElementsByClassName("name");
                    if(names[0].value) {
                        uArr[u++] = JSON.stringify({"id": row.cells[1].innerHTML, "access_level": names[0].value,
                            "status": names[1].value});
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("access_control.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
