...
@section('content')
            ...
            <div class="col-md-7 col-md-offset-2">
                <div class="table-responsive-sm" style="overflow-x:auto; overflow-y:auto;">
                    <div style="text-align:left">
                        {!! $empDesigs['empDesignations']->links() !!}
                    </div>
                    <table cellspacing="0" rules="all" border="1" id="Table1" style="border-collapse:collapse;" class="table table-hover table-bordered" >
                        <thead>
                            ...
                            <tr align='center'>
                                ...
                                <th width="20%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="name" style="cursor: pointer"> Name <span id="name_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="15%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="emp_type" style="cursor: pointer"> Emp Type <span id="emp_type_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="15%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="shift_type" style="cursor: pointer"> Shift Type <span id="shift_type_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="34%" align='center'> In Use(count) </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr align='center'>
                            @if($empDesigs['empDesignations'])
                                <?php $index = 2; ?>
                                @foreach ($empDesigs['empDesignations'] as $empDesignation)
                                    <?php $index++; ?>
                                    <tr>
                                        @if($empDesignation->in_use == 0)
                                        ...
                                        @endif
                                        <td>{{ $empDesignation->id }}</td>
                                        <td><label class="lblname">{{$empDesignation->name}}</label></td>
                                        @php
                                            $emp_type_index = null;
                                            foreach($empDesigs['empCategories'] as $key => $value){
                                                if($value == $empDesignation->emp_type){
                                                    $emp_type_index = $key;
                                                    break;
                                                }
                                            }
                                        @endphp
                                        <td><label class="lblname" for="{{$emp_type_index}}">{{$empDesignation->emp_type}}</label></td>
                                        @php
                                            $shift_type_index = null;
                                            foreach($empDesigs['shiftCategories'] as $key => $value){
                                                if($value == $empDesignation->shift_type){
                                                    $shift_type_index = $key;
                                                    break;
                                                }
                                            }
                                        @endphp
                                        <td><label class="lblname" for="{{$shift_type_index}}">{{$empDesignation->shift_type}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($empDesignation->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $empDesignation->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
                    ...
                    <div hidden>
                        <form id="searchfilter" method="POST" action="{{route('emp_designation.fetch_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
                        ...
                        <form id="cud_action" method="POST" action="{{route('emp_designation.update_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("emp_designation.fetch_data") }}?page=' + page;
        ...
        function add(){
            ...
            grid.insertCell(3).innerHTML = "{{Html::decode(Form::select('emp_type', $empDesigs['empCategories'], null, array('class' =>'name')))}}";
            grid.cells[3].innerHTML = grid.cells[3].innerText;

            grid.insertCell(4).innerHTML = "{{Html::decode(Form::select('shift_type', $empDesigs['shiftCategories'], null, array('class' =>'name')))}}";
        ...
        function edit(element, index){
            ...
            if(element.innerHTML.search("pencil") != -1){
                ...
                lblName = lblelement[0].innerText;
                cells[3].innerHTML = "{{Html::decode(Form::select('emp_type', $empDesigs['empCategories'], null, array('class' =>'name')))}}";
                cells[3].innerHTML = "<label class='lblname' for='" + lblIndex + "' hidden>" + lblName + "</label>" + cells[3].innerText;
                var emp_type = cells[3].getElementsByClassName("name");
                emp_type[0].options[lblIndex].selected = true;

                lblelement = cells[4].getElementsByClassName("lblname");
                lblIndex = lblelement[0].htmlFor;
                lblName = lblelement[0].innerText;
                cells[4].innerHTML = "{{Html::decode(Form::select('shift_type', $empDesigs['shiftCategories'], null, array('class' =>'name')))}}";
                cells[4].innerHTML = "<label class='lblname' for='" + lblIndex + "' hidden>" + lblName + "</label>" + cells[4].innerText;
                var shift_type = cells[4].getElementsByClassName("name");
                ...
                lblIndex = lblelement[0].htmlFor;
                lblName = lblelement[0].innerText;
                cells[4].innerHTML = "<label class='lblname' for='" + lblIndex + "' >" + lblName + "</label>";
        ...
        function save(){
            ...
            for (var k = 0, n = 0, u = 0, d = 0, l = 0; k < actions.length; k++) {
                ...
                if (actions[k].innerHTML == 'L') {
                    ...
                    if(names[0].value) {
                        nArr[n++] = JSON.stringify({"name": names[0].value,
                        "emp_type": names[1].options[names[1].selectedIndex].text,
                        "shift_type": names[2].options[names[2].selectedIndex].text});
                    }
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("emp_designation.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
