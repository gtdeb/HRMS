...
@section('content')
                            ...
                            @if($payrollPayableData['payrollPayables'])
                                ...
                                @foreach ($payrollPayableData['payrollPayables'] as $payrollPayable)
                                    ...
                                    <tr>
                                        ...
                                        <td><label class="lblname">{{$payrollPayable->ot}}</label></td>
                                        <td><label class="lblname">{{$payrollPayable->leave_encashment}}</label></td>
                                        <td><label class="lblname">{{$payrollPayable->allowance}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($payrollPayable->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $payrollPayable->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("payroll_payable.fetch_data") }}?page=' + page;
        ...
        function add(){
            ...
            grid.insertCell(7).innerHTML = "<input class='name' type='number' min='00.00' step='00.01' value='00.00' style='border:none'>";
            grid.insertCell(8).innerHTML = "<input class='name' type='number' min='00.00' step='00.01' value='00.00' style='border:none'>";
            grid.insertCell(9).innerHTML = "";
        }
        ...
        function edit(element, index){
            ...
            if(element.innerHTML.search("pencil") != -1){
                ...
                lblName = lblelement[0].innerText;
                cells[7].innerHTML = "<label class='lblname' hidden>" + lblName + "</label><input class='name' type='number' min='00.00' step='00.01' style='border:none' value='" + lblName +  "'>";

                lblelement = cells[8].getElementsByClassName("lblname");
                lblName = lblelement[0].innerText;
                cells[8].innerHTML = "<label class='lblname' hidden>" + lblName + "</label><input class='name' type='number' min='00.00' step='00.01' style='border:none' value='" + lblName +  "'>";
            }
            else{
                if(chkbx[0]) {
        ...
        function save(){
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("payroll_payable.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
