...
@section('content')
        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <div class="table-responsive-sm" style="overflow-x:auto; overflow-y:auto;">
                    ...
                    <table cellspacing="0" rules="all" border="1" id="Table1" style="border-collapse:collapse;" class="table table-hover table-bordered" >
                        <thead>
                            ...
                            <tr align='center'>
                                ...
                                <th width="27%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="name" style="cursor: pointer"> Name <span id="name_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="27%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="CF" style="cursor: pointer"> Carry Forward <span id="CF_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                            ...
                            @if($leaveCategories)
                                ...
                                @foreach ($leaveCategories as $leaveCategory)
                                    ...
                                    <tr>
                                        ...
                                        <td><label class="lblname">{{$leaveCategory->name}}</label></td>
                                        <td><label class="lblname">{{$leaveCategory->CF}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($leaveCategory->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $leaveCategory->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
                    ...
                    <div hidden>
                        <form id="searchfilter" method="POST" action="{{route('leave_category.fetch_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
                        ...
                        <form id="cud_action" method="POST" action="{{route('leave_category.update_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("leave_category.fetch_data") }}?page=' + page;
        ...
        function add(){
            ...
            grid.insertCell(3).innerHTML = "<select id='CF' class='name' style='text-align:center;height:20px;'>" +
        ...
        function edit(element, index){
            ...
            if(element.innerHTML.search("pencil") != -1){
                ...
                lblName = lblelement[0].innerText;
                //alert(lblName);
                cells[3].innerHTML = "<label class='lblname' hidden>" + lblName + "</label><select id='CF' " +
                    "class='name' style='text-align:center;height:20px;'><option value='No'>No</option>" +
                    "<option value='Yes'>Yes</option></select>";
        ...
        function save(){
            ...
            for (var k = 0, n = 0, u = 0, d = 0, l = 0; k < actions.length; k++) {
                ...
                if (actions[k].innerHTML == 'L') {
                    ...
                    if(names[0].value) {
                        nArr[n++] = JSON.stringify({"name": names[0].value, "CF": names[1].value});
                    }
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("leave_category.update_data") }}?page=' + $('#fd_cud_page').val();
