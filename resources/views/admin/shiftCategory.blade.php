...
@section('content')
                            ...
                            @if($shiftCategories)
                                ...
                                @foreach ($shiftCategories as $shiftCategory)
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <td>{{ $shiftCategory->id }}</td>
                                        <td><label class="lblname">{{$shiftCategory->name}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($shiftCategory->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $shiftCategory->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("shift_category.fetch_data") }}?page=' + page;
        ...
        function save(){
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("shift_category.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
