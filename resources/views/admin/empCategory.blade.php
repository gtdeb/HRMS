...
@section('content')
        ...
        <div class="row">
            <div class="col-md-5 col-md-offset-3">
                <div class="table-responsive-sm" style="overflow-x:auto; overflow-y:auto;">
                    ...
                    <table cellspacing="0" rules="all" border="1" id="Table1" style="border-collapse:collapse;" class="table table-hover table-bordered" >
                        <thead>
                            ...
                            <tr align='center'>
                                ...
                                <th width="20%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="id" style="cursor: pointer"> Sr <span id="id_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="30%" align='center' class="sorting" data-sorting_type="{{$pageSetting['sort_type']}}" data-column_name="name" style="cursor: pointer"> Name <span id="name_sort_icon"><i class="fa fa-sort" aria-hidden="true"></i></span></th>
                                <th width="40%" align='center'> In Use(count) </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr align='center'>
                            @if($empCategories)
                                ...
                                @foreach ($empCategories as $empCategory)
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <td>{{ $empCategory->id }}</td>
                                        <td><label class="lblname">{{$empCategory->name}}</label></td>
                                        <td>
                                            @php
                                                $in_use = 'No';
                                                if($empCategory->in_use > 0){
                                                ...
                                                $in_use = $in_use . '(' . $empCategory->in_use . ')';
                                            @endphp
                                            {{ $in_use }}
                    ...
                    <div hidden>
                        <form id="searchfilter" method="POST" action="{{route('emp_category.fetch_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
                        ...
                        <form id="cud_action" method="POST" action="{{route('emp_category.update_data',null,1)}}" enctype="application/json">
                            {{csrf_field()}}
    ...
    <script type="text/javascript">
        ...
        function fetch_data(page, recs, sort_type, sort_by, query)
        {
            ...
            $('#fd_query').val(query);
            document.getElementById("searchfilter").action = '{{ route("emp_category.fetch_data") }}?page=' + page;
        ...
        function save(){
            ...
            if (qIndx && confirm(message)) {
                ...
                document.getElementById("cud_action").action = '{{ route("emp_category.update_data") }}?page=' + $('#fd_cud_page').val();
                $("#waitingScreen").modal("show");
                $("#cud_action").submit();
            }
        }
